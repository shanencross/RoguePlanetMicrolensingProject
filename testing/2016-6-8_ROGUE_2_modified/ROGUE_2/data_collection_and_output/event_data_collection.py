...
MOA_dir = "http://www.massey.ac.nz/~iabond/moa/alert" + event_year_MOA + "/"
if DEBUGGING_MODE:
...
def convert_event_name(event_name_initial):
	...
	if survey == "MOA":
		initial_survey = "MOA"
		final_survey = "OGLE"
		event_year = event_year_MOA
	elif survey == "OGLE":
		initial_survey = "OGLE"
		final_survey = "MOA"
		event_year = event_year_OGLE
	else:
	...
	cross_reference_URL = MOA_dir + "/fetchtxt.php?path=moa/alert" + event_year + "/moa2ogle.txt"
	cross_reference_request = requests.get(cross_reference_URL, verify=False)
	cross_reference_soup = BeautifulSoup(cross_reference_request.content, "lxml")
	lines = cross_reference_soup.get_text().split("\n")
